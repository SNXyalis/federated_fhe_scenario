...
FHEClient new_lead_client() {
    ...
    auto evalMultKey = cc->KeySwitchGen(s.secretKey, s.secretKey);
    cc->EvalSumKeyGen(s.secretKey);
    auto evalSumKeys =
        std::make_shared<std::map<usint, EvalKey<DCRTPoly>>>(cc->GetEvalSumKeyMap(s.publicKey->GetKeyTag()));
...
void new_client_add_multiplication_key(FHEClient c) {
    c.receive_server_publickey();
    c.send_joined_evalMultKey(c.receive_evalMultKey(), c.receive_key_switch());
}

//Key Generation process
void init_clients(int num_of_clients) {
    ...
    EvalKey<DCRTPoly> server_mult_key = s.clientCC->MultiMultEvalKey(s.secretKey, server_keyswitch, server_pk->GetKeyTag()); //start multiplication key generation
    s.send_key_switch(server_keyswitch);
    s.send_evalMultKey(server_mult_key);

    for ( int i = 0; i < num_of_clients; i++) {
        new_client_add_multiplication_key(v[i]);
    }
...
class Aggregator {
    public:
        ...
        void send_key_switch(EvalKey<DCRTPoly> evalMultKey) {
            if (!Serial::SerializeToFile(DATAFOLDER + "/key-switch.txt", evalMultKey, SerType::BINARY)) {
                std::cerr << "Error writing serialization of keyswitch key to key-public.txt" << std::endl;
            }
        }
