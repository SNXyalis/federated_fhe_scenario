...
void RunCKKS() {
    ...
    //Client
    auto res = c.receive_ct_for_decryption();
    c.send_partial_decrypted_ct( res );
    
    //Server
    partialCiphertextVec.push_back(s.receive_partial_decrypted_ct());

    //Client1
    auto res1 = c1.receive_ct_for_decryption();
    c1.send_partial_decrypted_ct( res );

    //Server
    partialCiphertextVec.push_back(s.receive_partial_decrypted_ct());

    //TODO clients are supposed to get that part
